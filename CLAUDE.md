...
- **Reste à vivre** (accueil), deux valeurs : **prévisionnel** = `revenus − total factures − budgets alloués − épargne prévue du mois` (`getPlannedSavingsForMonth`, tous virements du mois) ; **réel** = `solde du mois − épargne réellement virée` (`getRealSavingsForMonth`, virements `done=1`).
